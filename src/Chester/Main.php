...
class Main extends PluginBase implements CommandExecutor, Listener
{
	...
	public function onEnable() {
		...
		$this->getServer()->getLogger()->info(TextFormat::RED."[Chester] Plugin loaded!");
    ...
    public function onCommand(CommandSender $sender, Command $cmd, $label, array $args) {
    	...
    	case "reload":
        	...
        	$this->chest=$this->config->get("chest");
        	$this->set=array();
    		$sender->sendMessage("[Chester] Reload successful!");
    		break;
    	case "reset":
    		$this->ResetChest();
    		$sender->sendMessage("[Chester] Chest has been refilled!");
    		break;
    	case "clear":
    		$this->ClearChest();
    		$sender->sendMessage("[Chester] Chests have been cleared.");
    		break;
    	case "add":
    	case "remove":
    		if(!$sender instanceof Player){$sender->sendMessage("Don't break it!");break;};
    		$this->set[$sender->getName()] = $args[0];
            $sender->sendMessage("[Chester] Click the chest to confirm.");
    		break;
    	default:
    ...
    public function onInteract(PlayerInteractEvent $event)
    {
        ...
        if(isset($this->set[$event->getPlayer()->getName()]))
        {
        	if($block->getId()!=54)
        	{
        		$event->getPlayer()->sendMessage("[Chester] Please tap a chest.");
            ...
            case "add":
            	foreach($this->chest as $key=>$val)
            	{
            		...
            		{
            			$event->getPlayer()->sendMessage("[Chester] This chest is already added.");
            			unset($event,$block,$key,$val);
            			return;
            		}
            	}
                ...
                $this->chest[]=$tmp;
                unset($tmp,$key,$val);
                $event->getPlayer()->sendMessage("[Chester] Adding to the reset list...");
                break;
            case "remove":
    ...
    public function onBreakEvent(BlockBreakEvent $event) {
    	...
    	foreach($this->chest as $key=>$val)
        {
        	if($val["x"]==$block->getX() && $val["y"]==$block->getY() && $val["z"]==$block->getZ() && $val["level"]==$block->getLevel()->getFolderName())
        	{
            	...
            	array_splice($this->chest,$key,1);
            	$event->getPlayer()->sendMessage("[Chester] Removing from reset list...");
            	break;
            }
        }
        unset($event,$block,$key,$val);
    ...
    public function onDisable() {
        //$this->saveChest();
        $this->getLogger()->warning("Turning off Chester!");
